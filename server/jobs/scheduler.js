...
cron.schedule('0 1 * * *', async () => { // FOR TESTING: Runs every 2 minutes
  ...
  try {
    // Loop through all users and generate insights for each
    ...
    for (const user of users) {
      ...
      if (entries.length >= 2) {
        console.log(`Found ${entries.length} entries for insight generation for user ${user._id}.`);
        ...
        console.log('Successfully generated and saved new AI insight for user', user._id);

      } else {
        console.log(`Not enough entries in the last 7 days (${entries.length} found) to generate insight for user ${user._id}.`);
      }
    }
  } catch (error) {
